...
def generate_launch_description():
    ...
    urdf_file_name = 'neuronbot2.urdf.xacro'    
    nb2_urdf = os.path.join(
        get_package_share_directory('neuronbot2_description'),
        'urdf',
        urdf_file_name)

    urdf_file_name = 'neuronbot2_w_front_camera.urdf.xacro'
    nb2_w_front_camera_urdf = os.path.join(
        get_package_share_directory('neuronbot2_description'),
        'urdf',
        urdf_file_name)        

    urdf_file_name = 'neuronbot2_w_top_camera.urdf.xacro'
    ...
    return LaunchDescription([
        ...
        Node(
            ...
            parameters=[
                {'use_sim_time': use_sim_time,
                 'robot_description': Command(['xacro ', nb2_urdf])}
            ]),
        ...
        Node(
            ...
            parameters=[
                {'use_sim_time': use_sim_time,
                 'robot_description': Command(['xacro ', nb2_w_front_camera_urdf])}
            ]),
        ...
        Node(
            ...
            parameters=[
                {'use_sim_time': use_sim_time,
                 'robot_description': Command(['xacro ', nb2_w_top_camera_urdf])}
            ]),
    ])
